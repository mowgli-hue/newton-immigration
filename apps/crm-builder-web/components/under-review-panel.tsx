...
export default function UnderReviewPanel({
  ...
  const reviewedBy = localReviewer || (urCase as any).reviewedBy || "";
  ...
  const reviewerNames = teamUsers.length > 0
    ? teamUsers.filter(u => u.active !== false).map((u: any) => u.name)
    : ["Ramandeep Kaur","Rajwinder Kaur","Avneet Kaur","Simi Das","Manisha","Rapneet Kaur","Sukhman","Serbleen"];
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 px-4 pb-0" onClick={onClose}>
      <div className="w-full max-w-md rounded-t-2xl sm:rounded-2xl bg-white shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex border-b border-slate-100 bg-slate-50">
          {[{n:1,l:"Reviewer"},{n:2,l:"Review"},{n:3,l:"Changes"},{n:4,l:"Submit"}].map(s => (
          ...
          {step === 1 && (
            <div className="space-y-3">
              ...
              <button onClick={async () => {
                ...
                setLocalReviewer(name); // stay open, jump to step 2
          ...
          {step === 2 && (
            <div className="space-y-3">
              ...
              <button onClick={async () => {
                ...
                onClose();
                setCaseActionStatus("⚠️ Changes sent to " + (urCase.assignedTo || "staff") + " — check CRM notes & case");
          ...
          {step === 3 && (
            <div className="space-y-3">
              ...
              <button onClick={async () => {
                ...
                onClose();
                setCaseActionStatus("✅ Done — " + ((urCase as any).reviewedBy || "reviewer") + " notified in CRM notes");
