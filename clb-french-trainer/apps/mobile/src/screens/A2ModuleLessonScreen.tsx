...
type Props = NativeStackScreenProps<MainStackParamList, 'A2ModuleLessonScreen'>;

function parseA2LessonNumber(lessonId: string): number | null {
  const match = lessonId.match(/^a2-lesson-(\d+)$/);
...
export function A2ModuleLessonScreen({ route, navigation }: Props) {
  ...
  const nextLessonId = useMemo(() => {
    ...
    return `a2-lesson-${lessonNumber + 1}`;
  }, [lessonNumber]);
    ...
    return (
      <View style={styles.root}>
        <View style={styles.container}>
          <Card>
            <Text style={styles.title}>A2 Lesson Route Error</Text>
            <Text style={styles.subtitle}>This screen is intended for A2 lessons 1 to 40.</Text>
  ...
  if (result) {
    ...
    return (
      <View style={styles.root}>
        <View style={styles.container}>
          <Card>
            <Text style={styles.badge}>A2 Lesson {lessonNumber} Result</Text>
            <Text style={styles.title}>{result.passed ? 'Lesson Complete' : 'Lesson Review Needed'}</Text>
            <Text style={styles.subtitle}>
              {result.passed
                ? isFinal
                  ? 'You completed the final lesson in this A2 module.'
