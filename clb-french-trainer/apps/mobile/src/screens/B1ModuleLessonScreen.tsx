...
type Props = NativeStackScreenProps<MainStackParamList, 'B1ModuleLessonScreen'>;

function parseB1LessonNumber(lessonId: string): number | null {
  const match = lessonId.match(/^b1-lesson-(\d+)$/);
  if (!match) return null;
  const num = Number(match[1]);
  return Number.isFinite(num) ? num : null;
}

export function B1ModuleLessonScreen({ route, navigation }: Props) {
  ...
  const nextLessonId = useMemo(() => {
    ...
    return `b1-lesson-${lessonNumber + 1}`;
  }, [lessonNumber]);
  ...
  const openNext = () => {
    ...
    (navigation.navigate as any)('B1ModuleLessonScreen', { lessonId: nextLessonId });
  };

  if (!lessonNumber || lessonNumber < 1 || lessonNumber > 40) {
    return (
      <View style={styles.root}>
        <View style={styles.container}>
          <Card>
            <Text style={styles.title}>B1 Lesson Route Error</Text>
            <Text style={styles.subtitle}>This screen supports B1 lessons 1 to 40.</Text>
  ...
  if (result) {
    ...
    return (
      <View style={styles.root}>
        <View style={styles.container}>
          <Card>
            <Text style={styles.badge}>B1 Lesson {lessonNumber} Result</Text>
            <Text style={styles.title}>{result.passed ? 'Lesson Complete' : 'Lesson Review Needed'}</Text>
            <Text style={styles.subtitle}>
              {result.passed
                ? isFinal
                  ? 'You completed the final lesson in this B1 module.'
                  : `Continue to B1 Lesson ${lessonNumber + 1}.`
                : 'Review missed tasks and complete the retry round before moving on.'}
            </Text>
            ...
            <View style={styles.actions}>
              {result.passed ? (
                <Button label={isFinal ? 'Open Module Review' : `Continue to B1 Lesson ${lessonNumber + 1}`} onPress={openNext} />
              ) : (
  ...
  return (
    <StructuredLessonScreen
      ...
      onComplete={({ passed, scorePercent, minorCorrection }) => {
        if (passed) {
          const base = Math.max(80, scorePercent);
          completeLesson({
            lessonId,
        ...
        if (passed) {
          if (user?.email) {
            void sendLessonCompletionEmail({
              ...
              lessonId,
              lessonTitle: `B1 Lesson ${lessonNumber}`,
              scorePercent,
